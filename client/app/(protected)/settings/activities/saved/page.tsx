...
import { getSavedItems, SavedHistoryItem, PaginatedResponse } from "@/features/activities/history.api";
import { MiniLoader } from "@/components/ui/mini-loader";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { Bookmark, Image, Film, MessageCircle, Heart, Eye, ArrowLeft } from "lucide-react";
import Link from "next/link";
import { toast } from "sonner";
import { getRelativeTime } from "@/components/ui/time-ago";

export default function SavedItemsPage() {
    const [data, setData] = useState<PaginatedResponse<SavedHistoryItem> | null>(null);
    ...
    const fetchSaved = useCallback(async (pageNum: number, append = false) => {
        try {
            const result = await getSavedItems(pageNum, 20);
            if (append && data) {
                setData({
                    ...result,
                ...
                {!data || data.items.length === 0 ? (
                    ...
                    <>
                        {data.items.map((item, index) => (
                            <SavedItemCard key={`${item.type}-${index}`} item={item} />
                        ))}
...
function SavedItemCard({ item }: { item: SavedHistoryItem }) {
                ...
                <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground/70 mt-1">
                            Saved {getRelativeTime(item.saved_at)}
                        </p>
                    </div>
