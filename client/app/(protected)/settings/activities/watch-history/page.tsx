...
import { getWatchHistory, WatchHistoryItem, PaginatedResponse } from "@/features/activities/history.api";
...
export default function WatchHistoryPage() {
    ...
    const fetchHistory = useCallback(async (pageNum: number, append = false) => {
        try {
            const result = await getWatchHistory(pageNum, 20);
            if (append && data) {
                setData({
                    ...result,
            ...
            toast.error(error?.response?.data?.message || "Failed to load watch history");
        }
    }, [data]);
            ...
            <div className="container max-w-3xl mx-auto px-4 py-6 space-y-3">
                {!data || data.items.length === 0 ? (
                    <div className="text-center py-16">
                        ...
                        <p className="text-lg font-medium text-muted-foreground">No watch history</p>
                        <p className="text-sm text-muted-foreground/70 mt-1">
                            Posts and reels you view will appear here
                        </p>
                    </div>
                ) : (
                    <>
                        {data.items.map((item, index) => (
                            <WatchHistoryCard key={`${item.type}-${index}`} item={item} />
                        ))}
