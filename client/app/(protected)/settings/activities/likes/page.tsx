...
import { getLikeHistory, LikeHistoryItem, PaginatedResponse } from "@/features/activities/history.api";
import { MiniLoader } from "@/components/ui/mini-loader";
import { Button } from "@/components/ui/button";
import { Card } from "@/components/ui/card";
import { Heart, Image, Film, MessageCircle, Eye, ArrowLeft } from "lucide-react";
import Link from "next/link";
import { toast } from "sonner";
import { getRelativeTime } from "@/components/ui/time-ago";

export default function LikeHistoryPage() {
    const [data, setData] = useState<PaginatedResponse<LikeHistoryItem> | null>(null);
    const [loading, setLoading] = useState(true);
    const [loadingMore, setLoadingMore] = useState(false);
    const [page, setPage] = useState(1);

    const fetchLikes = useCallback(async (pageNum: number, append = false) => {
        try {
            const result = await getLikeHistory(pageNum, 20);
            if (append && data) {
                setData({
                    ...result,
            ...
            console.error("Failed to fetch like history:", error);
            toast.error(error?.response?.data?.message || "Failed to load like history");
        }
    }, [data]);
    ...
    const handleLoadMore = async () => {
        ...
        setLoadingMore(true);
        await fetchLikes(nextPage, true);
        setPage(nextPage);
                ...
                <div className="flex items-center gap-3">
                    ...
                    {data && (
                        <span className="text-sm text-muted-foreground ml-auto">
                            {data.total} {data.total === 1 ? "like" : "likes"}
                        </span>
                    )}
                </div>
            </header>
                ...
                {!data || data.items.length === 0 ? (
                    ...
                    <>
                        {data.items.map((item, index) => (
                            <LikeHistoryCard key={`${item.type}-${index}`} item={item} />
                        ))}
...
function LikeHistoryCard({ item }: { item: LikeHistoryItem }) {
                ...
                <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-muted-foreground/70 mt-1">
                            Liked {getRelativeTime(item.liked_at)}
                        </p>
                    </div>
